import { StyleSheet } from 'react-native';
import { colors, radii, spacing, typography } from '../../constants/theme';

const styles = StyleSheet.create({
  headerPad: { paddingHorizontal: spacing.screen },
  todayTop: {
    height: 82,
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
  },
  eyebrow: {
    ...typography.caption,
    color: colors.primary,
    fontWeight: '700',
    letterSpacing: 1.4,
  },
  dateTitle: { ...typography.headingXL, color: colors.text },
  plus: {
    width: 44,
    height: 44,
    borderRadius: 22,
    backgroundColor: colors.primary,
    alignItems: 'center',
    justifyContent: 'center',
  },
  dateStrip: {
    height: 72,
    paddingHorizontal: spacing.small,
    flexDirection: 'row',
    borderTopWidth: StyleSheet.hairlineWidth,
    borderBottomWidth: StyleSheet.hairlineWidth,
    borderColor: colors.divider,
  },
  dateCell: {
    flex: 1,
    alignItems: 'center',
    justifyContent: 'center',
    gap: 3,
    borderBottomWidth: 3,
    borderBottomColor: colors.transparent,
  },
  dateActive: { borderBottomColor: colors.primary },
  dayName: { fontSize: 10, color: colors.muted, fontWeight: '700' },
  dayNumber: { ...typography.bodyLarge, color: colors.textSecondary },
  activeText: { color: colors.text },
  content: { paddingHorizontal: spacing.screen, paddingBottom: 110 },
  filters: { gap: spacing.small, paddingVertical: spacing.large },
  filter: {
    height: 40,
    flexDirection: 'row',
    alignItems: 'center',
    gap: spacing.small,
    paddingHorizontal: spacing.medium,
    borderRadius: 20,
    backgroundColor: colors.surface,
  },
  filterActive: { backgroundColor: colors.selectedBlue },
  filterText: {
    ...typography.caption,
    color: colors.textSecondary,
    fontWeight: '700',
  },
  sectionRow: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
    marginBottom: spacing.medium,
  },
  sectionTitle: {
    ...typography.bodyLarge,
    color: colors.text,
    letterSpacing: 1,
  },
  count: { ...typography.caption, color: colors.textSecondary },
  habitList: { gap: spacing.medium },
  empty: { alignItems: 'center', paddingVertical: spacing.xxl },
  emptyTitle: {
    ...typography.headingM,
    color: colors.text,
    marginTop: spacing.medium,
  },
  emptyText: {
    ...typography.body,
    color: colors.textSecondary,
    marginTop: spacing.xs,
    textAlign: 'center',
  },
  create: { marginTop: spacing.xl },
  backdrop: { flex: 1, backgroundColor: colors.overlay },
  popover: {
    position: 'absolute',
    right: spacing.screen,
    top: 180,
    width: 190,
    padding: spacing.small,
    borderRadius: radii.button,
    backgroundColor: colors.surfaceSecondary,
    shadowColor: '#000',
    shadowOpacity: 0.35,
    shadowRadius: 12,
    elevation: 12,
  },
  menuItem: {
    height: 48,
    flexDirection: 'row',
    alignItems: 'center',
    gap: spacing.medium,
    paddingHorizontal: spacing.medium,
  },
  menuText: {
    ...typography.caption,
    color: colors.text,
    fontWeight: '700',
    letterSpacing: 0.7,
  },
  sheetBackdrop: {
    flex: 1,
    backgroundColor: colors.overlay,
    justifyContent: 'flex-end',
  },
  sheet: {
    padding: spacing.screen,
    paddingBottom: 38,
    gap: spacing.large,
    borderTopLeftRadius: 24,
    borderTopRightRadius: 24,
    backgroundColor: colors.surface,
  },
  sheetHandle: {
    width: 42,
    height: 4,
    alignSelf: 'center',
    borderRadius: 2,
    backgroundColor: colors.muted,
  },
  sheetTop: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
  },
  sheetTitle: { ...typography.headingM, color: colors.text },
  form: { paddingBottom: 40, gap: spacing.large },
  formIntro: {
    ...typography.body,
    color: colors.textSecondary,
    marginBottom: spacing.small,
  },
  label: {
    ...typography.caption,
    color: colors.textSecondary,
    fontWeight: '700',
    letterSpacing: 1,
  },
  iconOptions: { flexDirection: 'row', gap: spacing.medium },
  iconOption: {
    width: 50,
    height: 50,
    borderRadius: 15,
    backgroundColor: colors.surfaceSecondary,
    alignItems: 'center',
    justifyContent: 'center',
  },
  iconActive: { backgroundColor: colors.selectedBlue },
  timeGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: spacing.small },
  timeOption: {
    width: '48%',
    height: 44,
    backgroundColor: colors.surfaceSecondary,
    borderRadius: radii.small,
    alignItems: 'center',
    justifyContent: 'center',
  },
  timeActive: { backgroundColor: colors.selectedBlue },
  timeText: {
    ...typography.caption,
    color: colors.textSecondary,
    fontWeight: '700',
  },
  reminder: {
    minHeight: 72,
    flexDirection: 'row',
    alignItems: 'center',
    gap: spacing.medium,
    padding: spacing.medium,
    borderRadius: radii.button,
    backgroundColor: colors.surface,
  },
  reminderIcon: {
    width: 40,
    height: 40,
    borderRadius: 13,
    backgroundColor: '#25344A',
    alignItems: 'center',
    justifyContent: 'center',
  },
  reminderCopy: { flex: 1 },
  reminderTitle: { ...typography.bodyLarge, color: colors.text },
  reminderHint: { ...typography.caption, color: colors.textSecondary },
  mockHint: { ...typography.caption, color: colors.muted },
  save: { marginTop: 'auto' },
  detailHero: {
    marginTop: spacing.large,
    borderRadius: 24,
    backgroundColor: colors.selectedBlue,
    padding: spacing.xl,
  },
  detailTitle: { ...typography.headingL, color: colors.text },
  detailTime: {
    ...typography.caption,
    color: '#D9E7FF',
    marginTop: spacing.small,
  },
  detailMetric: { marginTop: 48 },
  metricNumber: { ...typography.metric, color: colors.text },
  metricLabel: { ...typography.caption, color: '#D9E7FF', fontWeight: '700' },
  noteCard: {
    marginTop: spacing.large,
    padding: spacing.large,
    borderRadius: radii.card,
    backgroundColor: colors.surface,
    gap: spacing.small,
  },
  noteText: { ...typography.body, color: colors.text },
});

export default styles;
